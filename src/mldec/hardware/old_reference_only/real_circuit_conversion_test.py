...
def convert_aer_to_sampler_format(aer_result, circuits):
    ...
    class MockRegisterBin:
        ...
        def to_bool_array(self):
            ...
            arr = []
            print(f"    Processing register {self.regname} with counts: {self.counts_dict}")
            for bitstring, count in self.counts_dict.items():
                ...
                if ' ' in bitstring:
                    ...
                    bits = bitstring
                print(f"      Bitstring '{bitstring}' -> bits '{bits}' for register {self.reg_index}, counts {count}")
    ...
    for i, circuit in enumerate(circuits):
        ...
        print(f"Circuit {i} counts: {reversed_counts}")
        print(f"Circuit {i} classical registers: {[creg.name for creg in circuit.cregs]}")
        ...
        for reg_index, creg in enumerate(cregs):
            regname = creg.name
            nbits = creg.size
            print(f"  Register {regname}: index {reg_index}, bits {nbits}")
